...
This tutorial introduces RNA-Seq analysis, using a paired RNA-seq experiment as
an example. The dataset, RNA-seq Study of COVID-19 Vaccination [@Odak2024-wy], 
investigates the impact of COVID-19 vaccination. It includes gene-level
read counts from blood samples taken both before (Pre) and 24 hours after
(Post) mRNA COVID-19 vaccination from the same individuals. This paired design
enables direct comparisons of gene expression changes within each person. The
pre-generated gene-level read counts for this dataset will be downloaded from
its corresponding GEO entry: [GSE247401](https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE247401).

![](./data/exprAnalysisRNAseq/overview_illustration.png)
<div align="center">Figure 1: Study design of Odak et al (2024).</div></br>

__Expected Outcomes__: We anticipate that our analysis will reveal the activation of immune-related
genes, particularly those involved in interferon response and antiviral
defense. This aligns with findings from the original study.

__Analysis Workflow:__ The analysis workflow of this tutorial includes the following steps: 

1. __Data Acquisition:__ Download gene-level read counts from GEO.
2. __Differential Expression Analysis:__ Identify differentially expressed genes (DEGs).
3. __Visualization:__ Create volcano plots and heatmaps to visualize expression results.
4. __Cluster Analysis:__ Group genes with similar expression profiles across samples.
5. __Interpretation:__ Conduct GO and Reactome pathway enrichment analysis to interpret results.

__Note:__ This workflow focuses on the downstream analysis and intentionally omits
the initial, resource-intensive steps of an RNA-Seq experiment: __(1)__ alignment of
RNA reads to a reference, and __(2)__ counting reads overlapping with annotation
features. These upstream steps are not essential for learning purposes, and
their exclusion is possible because GEO provides precomputed gene-level read
count tables for the chosen dataset.
...
3. Gain practical experience with real count data, normalization techniques, and visualization methods (volcano plots and heatmaps).
...
2. Next click the <span style="background-color: #f1c40f;">Posit/RStudio Studio Cloud assignment link</span></a> provided under the same name
   on the corresponding assignment page on Canvas
...
Posti/RStudio, and then selecting `Knit to HTML`. This will not only execute
the code, but also regenerate this HTML document and update all output, tables
and figures generated by the evaluated R code chunks accordingly.  Similarly,
one can generate a PDF document instead. The environment that makes this
possible is called [R Markdown](https://rmarkdown.rstudio.com/). Importantly,
R Markdown files including interactive features, such as interactive tables used in the
below _Filter DEGs_ section, can only be rendered in a fully functional form 
to HTML. PDF or MS Word formats do not support interactive features.
...
foundation for the plots and enrichment analysis that follow. The last
line returns the first 100 rows of the DEG results in an interactive table for 
inspection.
...
(pre vs post). Note: this is a simplified design formula for demonstration purposes
of this study. _[Line 2]_ Removes genes with very low total counts (fewer than
...
This section adds biological meaning to the `res_tbl` table from `DESeq2` by adding
...
and saves them to a tabular file under `./results/exprAnalysisRNAseq/` for reporting, 
A slice of that table is shown as well, but only the first 100 rows!
Next, a normalized expression matrix (`vsd_mat`) is created that will be used for 
clustering and heatmap visualization in the next steps. 

```{r chunk6, eval=TRUE, message=FALSE}
res_annot <- annotate_genes_simple(res_tbl)  # adds gene symbols and functional description
res_annot <- res_annot[order(res_annot$padj),] # Sort by padj column
write.csv(res_annot, # Writes full result table to a file 
          paste0(result_dir, out_prefix, "_DESeq2_results_annotated.csv"),
          row.names = FALSE)
DT::datatable(pretty_num(res_annot[1:100, c(10, 8, 1:5)]), options = list(scrollX = TRUE)) # To show full table, remove [1:100, ]
...
The last line above (with `assays`) creates a variance-stabilized expression matrix
...
interpretation analyses, including GO or Reactome enrichment analysis.

```{r chunk6a, eval=TRUE, message=FALSE}
sig_genes <- subset(res_annot, abs(log2FoldChange) > 1 & padj < 0.05)
sig_genes <- sig_genes[order(sig_genes$padj),] # Sort by padj column
DT::datatable(pretty_num(sig_genes[,c(10, 8, 1:5)]), options = list(scrollX = TRUE))
```
...
expression of the top most significantly changed genes differs before and
...
samples with similar expression patterns. Pearson correlation coeffients are
used as similarity metric for clustering in both dimensions. The resulting
heatmap displays both row-wise (gene) and column-wise (sample) dendrograms,
showing how genes and samples cluster based on Pearson correlation. The cell
colors represent relative expression levels after row-wise scaling (scale =
"row"), which enhances contrast and makes it easier to see which genes are up-
or down-regulated in each sample. In this COVID-19 vaccination dataset,
...
Note: For visibility the heatmap is plotted only for the top 30 DEGs obtained 
stored in the `sig_genes` table generated under the _Filter DEGs_ section above. 
To include all `r nrow(sig_genes)` from `sig_genes` in the heatmap, remove
in the below code the subsetting with `[1:30]`.

```{r chunk7a, eval=TRUE, message=FALSE}
sig_gene_ids <- sig_genes[order(sig_genes$padj), "symbol"] # Extract gene symbols sorted by padj
sig_gene_ids <- sig_gene_ids[1:30] # Subset to top 30 for better visibility in heatmap
mat <- vsd_mat[sig_gene_ids, , drop = FALSE] # Subset rows in vsd_mat to sig_gene_ids
colnames(mat) <- paste(coldata$subject, coldata$time, sep = "_") # Assign column proper column names 
annotation_col <- data.frame(time = coldata$time, subject = coldata$subject, row.names = colnames(mat)) 
ann_colors <- list(time = c(pre = "#3182BD", post = "#E41A1C")) # Optional: custom colors for 'time'
pheatmap(mat,
         scale = "row",                           
         ...
         fontsize_row = 7,
         fontsize_col = 9,
         main = paste("Top", length(sig_gene_ids), "DEGs"))
```
...
that pathway and tells R to open it, similar to clicking a link. Clicking this 
[R-HSA-913531 pathway](https://reactome.org/PathwayBrowser/#/R-HSA-913531) link
will open the same Reactome pathway for testing without running the code.
...
participate in that biological process. When exploring these pathways, one
should look for:

+ where significant genes appear,
+ whether most are up-regulated (immune activation) or down-regulated and
...
```{r chunk9c, eval=FALSE, message=FALSE}
top_path <- er_readable@result$ID[1]
browseURL(paste0("https://reactome.org/PathwayBrowser/#/", top_path))
```

Reactome's pathways are highly useful for biology and biomedical research
because they provide a curated, organized view of biological processes and
molecular interactions, offering a clear framework for learning complex
cellular mechanisms. In the context of large-scale data, Reactome helps
students interpret experimental results by linking differentially expressed
genes or proteins to specific, well-defined biological pathways, enabling
functional analysis and interpretation of the 'omics' data.
