...
Because NJ produces an unrooted tree, it is then rooted
either by the midpoint method (placing the root halfway between the two most
distant tips) or by specifying a known outgroup sequence. In this example only the
midpoint method is used. How to perform rooting with an outgroup is show but 
not evaluated. 


```{r example7a, eval=TRUE, message=FALSE}
# Midpoint rooting 
...
The final tree is plotted as a `"phylogram"` with the branch lengths proportional to evolutionary 
distance and bootstrap percentages shown at the internal nodes. The latter indicate how strongly 
each clade is supported by the data.
...
```{r example7c, eval=TRUE, message=FALSE}
