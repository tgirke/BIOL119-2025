...
![Identifier Download](NCBI_Protein.png "Downloading List of Identifiers")
...
origMAlign <- readDNAMultipleAlignment(filepath = system.file("extdata",
              "msx2_mRNA.aln", package = "Biostrings"), format = "clustal") # View multiple alignment in R
origMAlign
```
...
ape_tree_file = "rentrez-msa_ClustalOmega-P450_UniProtKB_SwissProt.ape_tree.bootstrap-gene_symbol.png" #for OUTPUT, not input
...
There are two lines that are removed, relative to `phyloAnalysis.R` (*png(ape_tree_file, height=1200, width=2000)* and *dev.off()*).  This is to enable visualization directly, without saving the image to a file.
